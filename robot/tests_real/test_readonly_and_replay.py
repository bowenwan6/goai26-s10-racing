...
ROOT = Path(__file__).resolve().parents[2]   # <repo>/robot/tests_real/
...
def test_no_motion_transport_or_launch_in_new_modules():
    ...
    for path in (ROOT / "robot" / "real_transfer").glob("*.py"):
...
def test_pending_profile_contains_no_fake_waypoints_or_identity_extrinsics():
    profile = json.loads((ROOT / "robot/real_transfer/config.pending.json").read_text())
