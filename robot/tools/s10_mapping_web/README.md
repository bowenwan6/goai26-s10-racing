...
2026-09-17新增：**独立“自动导航测试”页已部署102/106**，入口 `/native-nav`。支持原厂状态只读检查、受门禁控制的原地模式切换、Start/B 路线任务、停止与报告；沿用我们的 follower/router。实机只读和软件检查通过，实际行走及路线验收仍待现场完成，详见[使用说明与验收边界](NATIVE_NAV_GUIDE_ZH.md)。原现场助手仍只采集证据。

2026-09-15新增：**IMU／停稳诊断已部署102/106**，支持实时曲线、60/180/300秒被动采样、历史和导出。另新增[每次停稳的13秒“静止基准复零”按钮](IMU_ZERO_REFERENCE_GUIDE_ZH.md)，仅显示本次相对参考，不写硬件校准、不改变原停稳门禁；新版部署及真实网页只读检查已通过，真机按钮采样待现场确认。详见[诊断使用说明](IMU_DIAGNOSTIC_GUIDE_ZH.md)。
...
手机连接 **<ROBOT_WIFI_SSID>**，打开 [103 热点入口](http://10.21.41.1:8080/)。用户名 `golai`，密码已单独交接。使用遥控器控制行走。完整操作和算法说明见 [48 号 SLAM 指南] `git show docs-archive-20260920:docs/S10_48_SLAM_ZH.md`。
...
9月11日20:34的`indoor_loop_01`复测已跑通手机建图、回环及保存：92关键帧，日志与文件一致的8条回环约束，60条状态心跳均报告同步正常。完整地图142,598点，后处理完成。首尾距离1.16 m与用户确认的实际停靠偏移一米多基本吻合，未做独立测量，不代表已测得绝对定位精度；见复测报告 `../../artifacts/s10-48-indoor-loop-20260911-203447/REVIEW.md`。
