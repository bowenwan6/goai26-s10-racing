...
python3 "${REPO_ROOT}/robot/scripts/patch_upstream.py" --check
python3 "${REPO_ROOT}/robot/scripts/extract_waypoints.py" --check
...
bundles = (
    (root / "models/deployed/gate16/climb_policy_manifest.json", ("base_onnx", "base_sha256")),
    (root / "models/deployed/gate16/climb_policy_manifest.json", ("residual_onnx", "residual_sha256")),
    (root / "models/deployed/stairs_stable/policy_manifest.json", ("onnx", "sha256")),
)
...
python3 "${REPO_ROOT}/robot/scripts/runtime_fingerprint.py" check \
