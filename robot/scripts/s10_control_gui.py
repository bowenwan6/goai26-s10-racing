...
def run_remote(host, user, password, setup, action, strength, duration, cancel, alive, report):
    ...
    try:
        ...
        client.close()
        if log:
            folder = Path(__file__).resolve().parents[1] / 'out/s10-control/gui'
...
def gui():
    ...
    root = tk.Tk()
    root.title('S10 · 48号机器人控制 · 100% / 5秒')
    root.geometry('700x640')
    root.minsize(680, 620)
    frame = ttk.Frame(root, padding=20)
    frame.pack(fill='both', expand=True)
    ttk.Label(frame, text='S10 低速控制', font=('Microsoft YaHei UI', 20)).pack(anchor='w')
    ttk.Label(frame, text='保持手柄可接管 · 按机身方向移动 · 每次点击仅执行一轮').pack(anchor='w', pady=(4, 14))
    ...
    ttk.Label(frame, text='100% 为满量程；反馈超过平移 0.40 m/s / 旋转 0.50 rad/s 会提前回零。').pack(anchor='w', pady=(8, 10))
    pad = ttk.Frame(frame)
    pad.pack(fill='x')
    ...
    def start(action):
        ...
        def job():
            try:
                ...
                inbox.put(dict(kind='error', text=str(exc)))
            finally:
    ...
    def tick():
        ...
        while not inbox.empty():
            ...
            if kind == 'status' and item.get('motion'):
                ...
                status.set(f"状态 {m['state']} · {state_name}    电量 {level}    高度 {m['height']*100:.1f} cm\n"
