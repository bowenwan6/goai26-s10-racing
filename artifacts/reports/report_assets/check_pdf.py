...
sheet.save(root/'report_assets/page_overview.png')
log=(root/'report_assets/PROJECT_TECHNICAL_ZH.log').read_text(errors='replace')
issues=re.findall(r'(?:Missing character:.*|Overfull \\[hv]box.*|! .*)',log)
audit=json.loads((root/'TECHNICAL_MODEL_AUDIT.json').read_text())
hashes=[audit['checkpoint_sha256']]+[x['sha256'] for x in audit['deployed_onnx'].values()]
result={'pages':len(pdf),'bookmarks':len(pdf.get_toc()),'source_text_fragments_checked':count,'missing_text_fragments':sorted(set(missing)),'hashes_present':all(norm(h) in normalized for h in hashes),'missing_glyph_markers':raw.count('\ufffd'),'out_of_page_text':bounds,'blank_pages':empty,'latex_errors_or_overflow':issues,'page_summary':pageinfo}
(root/'report_assets/pdf_check.json').write_text(json.dumps(result,ensure_ascii=False,indent=2))
(root/'report_assets/pdf_text.txt').write_text(raw)
