"""Lifecycle adapter for the SDK-local 57D continuous stair-ascent actor.

The ONNX inference stays in ``rl_deploy`` so the actor receives the exact 57D observation
and recurrent history used during training.  This module only describes ownership,
entry limits and independent physical completion checks.
"""
...
class Stairs57Policy:
    ...
    # The SDK actor can be armed while rolling.  This flag only relaxes the speed gate for
    # deterministic segment tests that spawn at rest; the router still requires a stable pose.
    ...
    def continue_segment(self) -> None:
        """Keep recurrent history while requiring a fresh upper-platform check."""
        self._clear_for = 0.0
    ...
    def completion_candidate(self, state, dt: float, entry_position) -> tuple[bool, str]:
        """Confirm that the chassis, not only a front wheel, reached the upper deck."""
    ...
    def succeed(self, reason: str = "upper platform verified") -> None:
