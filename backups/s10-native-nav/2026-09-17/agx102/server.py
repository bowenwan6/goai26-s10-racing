...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path in ('/localization', '/heightmap', '/field', '/imu-check'):
            page = HERE/(('imu_diag.html' if self.path == '/imu-check' else self.path[1:]+'.html') if self.authenticated() else 'index.html')
            if not page.is_file():
