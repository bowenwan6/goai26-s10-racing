...
private:
    ...
    MatXf joint_cmd_;
    double stand_duration_ = cp_ptr_->stand_duration_;
...
public:
    ...
    virtual void Run() {
        ...
        if(run_time_ - time_stamp_record_ <= stand_duration_){
            ...
            float dt = 0.005;
            if (cp_ptr_->policy_stand_pose_.size() == 16) {
                // Finish the existing second stage at the model's exact joint pose.
                for (int i = 0; i < 16; ++i) {
                    planning_joint_pos(i) = GetCubicSplinePos(goal_joint_pos_(i), 0,
                        cp_ptr_->policy_stand_pose_(i), 0, new_time, stand_duration_);
                    planning_joint_vel(i) = i % 4 == 3 ? 0 : GetCubicSplineVel(
                        goal_joint_pos_(i), 0, cp_ptr_->policy_stand_pose_(i), 0,
