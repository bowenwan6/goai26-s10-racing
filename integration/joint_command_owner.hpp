...
class JointCommandOwner {
 public:
  ...
  MatXf Arbitrate(const MatXf& official, const MatXf* gate16,
                  const MatXf* stairs57, const VecXf& measured_pos,
                  bool* reset_official) {
    ...
    if (moving_gate16_request) {
      ...
      // The 57D actor is trained with continuous rolling contact. Transfer its first finite
      // action atomically, without inserting a stop on the first riser.
