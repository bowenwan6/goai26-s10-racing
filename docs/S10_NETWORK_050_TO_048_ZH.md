...
50 号手机访问失败的历史处理：原 PRO-050 热点下，iPhone/Mac 访问网页曾超时；抓包确认请求到达 AGX、应答也发回热点，但 TCP 握手未完成，根因没有确定。后来在 AGX 开设 `S10-Capture-050` 独立热点，手机直接连接 AGX 并访问 `http://10.42.50.1:8091/`，用户确认 iPhone 可用，随后设置热点和采集服务开机自启。这是绕开原故障路径，不能用 48 号后来发现的回程路由问题反推当时原因。
...
| 检查项 | 48 号结果 |
|---|---|
| 设备、SSID 与 SSH 身份记录 | 用户确认 48 号；SSID 实测为 `S10 PRO-048-5G`；103 通过已有 known_hosts 检查，AGX 首次记录到独立别名 `s10-48-agx` |
...
```text
电脑 10.21.41.19
    → S10 PRO-048-5G / 103 wlan1：10.21.41.1
