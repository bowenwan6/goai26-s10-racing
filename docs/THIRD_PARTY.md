...
Organizer material is cloned into the ignored `upstream/` directory and is not redistributed in
the submission ZIP. `scripts/setup_upstream.sh` checks out the pinned revision and applies the
submission's integration patch. Course coordinates in `src/s10_bringup/config/course.yaml` are a
generated configuration derived from the official scene; the generator verifies them against the
