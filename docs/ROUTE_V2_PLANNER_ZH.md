...
- 真实路线 `wt-wp-match/tools/wp_match/out/route_v2.json`（30 WP / 29 段 / 256.6 m / z −0.46→5.88 m）：可加载；每个中心线点投影回自身；带 5 cm 噪声与整段走廊宽度正弦偏移的连续跟踪无跳变、无层错配（路线自身最近的非相邻段水平距离 3.6 m）。测试通过 `S10_ROUTE_V2` 环境变量或默认路径定位，缺失时跳过。
