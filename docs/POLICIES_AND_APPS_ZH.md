...
- **真机自主导航首次成功（09-20，048 号狗）：** 室内地图上 WP01 → WP02，4.7 m，探测速度 0.10 m/s，跑到 `DONE`。用的是我们的路线跟随器（跑在 ROS 1 上）+ 原厂平地步态。台阶、长路线、更高速度都还没做。
...
| **1150** | 59→16，命令门控相位 1.5 Hz | 台阶、陡坡、横坡，`rl_nav` 的 `stairs_stable` 槽 | 🧪 | 复现包 `policies/1150_stairs.onnx` |
