...
> 33 evaluator waypoints in strict order in 436.058 seconds of simulation time. The successful
...
- WP28 and WP30 can retain committed motion where stacked-storey projection confuses the local
  height map.
...
Organizer scene/model/waypoint material is not redistributed; it is pulled from the access-
controlled upstream revision. Raw logs, telemetry, replay frames and videos are retained outside
the competition Git repository. Model and dependency provenance, including the unresolved
contributor-license action for team-supplied policy assets, is disclosed in `THIRD_PARTY.md`.
...
| Official WP32 simulation time | 436.059 s |
| Official elapsed | **436.058 s (7:16.058)** |
| Recorder wall elapsed | 708.40 s |
| Distance travelled | 257.49 m |
| Final WP32 distance | 0.177 m |
| Maximum recorded tilt | 57.9° |
| Recorder stall events | 3 |
...
|---:|---:|---:|---:|---:|---:|
| 0 | 0.001 | 11 | 96.498 | 22 | 220.810 |
| 1 | 13.152 | 12 | 105.211 | 23 | 238.230 |
| 2 | 20.935 | 13 | 116.787 | 24 | 280.653 |
| 3 | 27.491 | 14 | 127.891 | 25 | 294.106 |
| 4 | 33.497 | 15 | 135.610 | 26 | 312.266 |
| 5 | 42.841 | 16 | 152.857 | 27 | 326.452 |
| 6 | 51.180 | 17 | 155.926 | 28 | 369.356 |
| 7 | 62.788 | 18 | 174.602 | 29 | 394.889 |
| 8 | 71.001 | 19 | 182.978 | 30 | 401.596 |
| 9 | 76.579 | 20 | 191.834 | 31 | 415.175 |
| 10 | 88.035 | 21 | 212.262 | 32 | 436.059 |
...
owner acknowledged as Gate 16; fallback entry with command 0.18 m/s, base yaw approximately −5.05°,
valid native-frame height map and measured edge peak 0.37691 m; five seconds of Gate 16 control;
four-wheel candidate; 0.35 s verification; residual disarmed; actual owner changed to safe hold and
then official in 0.259 s; follower transient state reset. WP16 scored at simulation time 152.857 s
and WP17 at 155.926 s.

### 9.5 Video evidence

The separately submitted file `wp0_to_wp32_seed6_720p_official.mp4` is a continuous simulation-time
render of the accepted run. It is H.264 High, 1280×720, 30 fps and 436.067 s long—within one frame
of the official elapsed time. SHA-256:
`291cbfdbc6f6b0100f42d304ffdd7b976d2d15557ae956cbfbfbdf9acaabba81`.
It overlays elapsed time, target waypoint, active policy, actual joint owner and pass events.

### 9.6 Negative evidence and repeatability limits

The same configuration is not universally successful. A macOS seed-8 run passed Gate 16 and later
fell on WP27→WP28 at about 63°. A server seed-8 repeat failed during Gate 16 near 69° despite
selecting the intended fallback. Seed 10 became stuck before WP29. These outcomes show that the
accepted run is valid evidence of feasibility but not a statistically established success rate.
...
### 11.4 Fact-check result on 2026-08-19

The finished document was checked against the current branch rather than only reviewed as prose:

- the navigation/perception/strategy suite passed 382/382 tests in the pinned container;
- the observation/training-contract suite passed 29/29 tests;
...
- the accepted raw log contained exactly 33 ordered upstream `[TRACK]` events and the documented
  Gate 16 owner lifecycle;
