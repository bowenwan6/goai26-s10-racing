> **Archived.** This is the README as it shipped with the August 2026 simulation-contest release (tags `v1.0-sim-release` / `sim-contest-submission`), kept verbatim for its run instructions, the Gate 16 contract and the September field notes. The current README is [at the repository root](../README.md).
...
| 想做什么 | 看这里 |
|---|---|
| 一页看懂我们有哪些 policy 和 app | [docs/POLICIES_AND_APPS_ZH.md](POLICIES_AND_APPS_ZH.md) |
| 仓库目录、分支规则、大文件 | [docs/REPO_GUIDE_ZH.md](REPO_GUIDE_ZH.md) |
| 新狗 + 新 SLAM（x_nav）接入计划 | [docs/NEW_SLAM_XNAV_INTEGRATION_ZH.md](NEW_SLAM_XNAV_INTEGRATION_ZH.md) |
...
| 9 月导航（`rl_nav`：J3100 步行 + 1150 台阶） | [docs/RL_ROUTE_ROBUST_PLAN_ZH.md](RL_ROUTE_ROBUST_PLAN_ZH.md) · [docs/ROUTE_V2_PLANNER_ZH.md](ROUTE_V2_PLANNER_ZH.md) |
| 原生步态导航 | [native_transfer/README_ZH.md](../native_transfer/README_ZH.md) · [docs/NATIVE_START_B_ACCEPTANCE.md](NATIVE_START_B_ACCEPTANCE.md) |
| HIM 模型实机接入 | [docs/S10_HIM_DEPLOYMENT.md](S10_HIM_DEPLOYMENT.md) |
| 已上传文件的索引 | [docs/GITHUB_FILE_INDEX_ZH.md](GITHUB_FILE_INDEX_ZH.md) |
...
- [GitHub 文件索引](GITHUB_FILE_INDEX_ZH.md)：app、v3 点云、MuJoCo 场景、真机适配、现场照片和报告的入口及上传范围。
...
- [48 号环境、账号与传感器](S10_48_SETUP_ZH.md)：使用本队 `golai`，Windows 登录 `ssh s10-48-golai`。
- [48 号官方 SLAM：当前状态、使用方法与算法](S10_48_SLAM_ZH.md)：手机连接 48 号 Wi-Fi，打开 `http://10.21.41.1:8080/`；103 转发至 AGX 网页，遥控器控制行走。
...
- [历史数据、运动验证、地图与 RL 参考结论](S10_DATA_RESEARCH_ZH.md)
- [录制复核与重建交接](S10_RECORDING_REVIEW_AND_RECONSTRUCTION_ZH.md)、[楼梯匹配方法](S10_STAIRS_MATCHING_GUIDE_ZH.md)、[高台匹配方法](S10_LEDGE_MATCHING_GUIDE_ZH.md)
- [基础步态平面匹配与软参考](S10_BASIC_FLAT_MATCHING_ZH.md)：5 段、34 秒 Isaac Sim 试验，3 段未摔倒，转向和侧向 2 段侧翻，尚未训练 RL。
...
- [50 号 106 SLAM 调查（历史）](S10_SLAM_106_RESEARCH_ZH.md)、[下一步验证事项](TODO.md)
...
[`docs/SUBMISSION.md`](SUBMISSION.md). Dependency, data and model provenance are disclosed in
[`docs/THIRD_PARTY.md`](THIRD_PARTY.md), and the required post-competition scope is described
in [`docs/OPEN_SOURCE_PLAN.md`](OPEN_SOURCE_PLAN.md). The complete implementation-linked
...
[`docs/SUBMISSION.md`](SUBMISSION.md) for the exact filename and checksum.
...
接口、构建、起身接管与验证结果见 [S10 HIM 接入说明](S10_HIM_DEPLOYMENT.md)。
