...
def sdk_feedback_error(data, now):
    value = data.get('sdk_mode')
    if value is None or now - value['updated'] > .5:
        return 'SDK mode feedback stale'
    if value['state'] != 100:
        return 'robot left external SDK mode'
...
def main(root, dry, direction=0, strength=.1, seconds=2, manual=False, handset=False, him=False):
    ...
    baseline_gids = set()
    physical_received = [0]
    def physical_command(msg):
        # The native DDS writer is dormant in SDK mode. Require a quiet bus
        # before takeover and throughout dry mode; live also guards SDK state.
        if dry or phase == 'preflight':
            physical_received[0] += 1
    ...
    def mode(msg):
        save('sdk_mode', msg, dict(state=msg.data.motion_state.state))
    ...
    subscriptions.append(node.create_subscription(
        JointsDataCmd, '/JOINTS_CMD', physical_command, qos_profile_sensor_data))
    subscriptions.append(node.create_subscription(
        MotionInfo, '/MOTION_INFO', mode, qos_profile_sensor_data))
    ...
    if not dry and not handset:
        def emergency_key(msg):
            if msg.data in ('G20_KEY_R2', 'G12_KEY_D'):
                handset_error[0] = 'handset requested damping'
                stop.set()
        subscriptions.append(node.create_subscription(
            String, '/GAMEPAD_KEY', emergency_key, qos_profile_sensor_data))
    ...
    def spin(guard=True):
        ...
        now = time.monotonic()
        if guard:
            if physical_received[0]:
                raise RuntimeError('physical joint commands received from outside this live trial')
            if stop.is_set() or now - heartbeat[0] > 1:
                raise RuntimeError(handset_error[0] or 'local heartbeat lost / cancelled')
            err = validate_feedback(data, now) or (sdk_feedback_error(data, now) if not dry else None)
            if err:
    ...
    try:
        ...
        while time.monotonic() < end:
            ...
            if validate_feedback(data, time.monotonic()) is None and (dry or sdk_feedback_error(data, time.monotonic()) is None):
                break
        else:
        ...
        physical_publishers = node.get_publishers_info_by_topic('/JOINTS_CMD')
        if any(p.node_name != '_CREATED_BY_BARE_DDS_APP_' for p in physical_publishers):
            raise RuntimeError('another SDK joint publisher exists; refusing concurrent control')
        baseline_gids.update(bytes(p.endpoint_gid) for p in physical_publishers)
        if physical_received[0]:
            raise RuntimeError('another controller is sending joint commands')
        os.environ['S10_POLICY_TRACE_PATH'] = str(output_dir / 'policy_trace.jsonl')
        os.environ['S10_POLICY_TRACE_STEPS'] = '300'
        os.environ['S10_POLICY_PATH'] = str(root / 'src/S10_sdk_deploy/policy/policy.onnx')
        os.environ['S10_POLICY_SLOT'] = '0'
        for name in ('S10_SECOND_POLICY_PATH', 'S10_DOWN_POLICY_PATH', 'S10_SPEEDTURN_POLICY_PATH'):
        ...
        if dry:
            if {bytes(p.endpoint_gid) for p in node.get_publishers_info_by_topic('/JOINTS_CMD')} != baseline_gids:
                raise RuntimeError('dry isolation failed: physical joint publishers changed')
            isolated = private + '/DRY_JOINTS_CMD'
            if len(node.get_publishers_info_by_topic(isolated)) != 1:
                raise RuntimeError('dry isolation failed: expected one private joint publisher')
            subscribers = node.get_subscriptions_info_by_topic(isolated)
            if any(s.node_name != node.get_name() for s in subscribers):
                raise RuntimeError('unexpected consumer of isolated joint commands')
            event('DRY_ROS_GRAPH_VERIFIED')
        while True:
            if handset:
                ...
                while not (stand_requested.is_set() and remote.centered(time.monotonic())):
                    ...
                    if end_requested.is_set():
                        completed = True
                        return
            if dry and not manual and not him:
                phase = 'dry_verified'
                observe(2)
            else:
                phase = 'stand'
                ...
                if not lie_requested.is_set():
                    ...
                    observe(.04 if dry and him else (1 if manual or him else 5))
                    if manual:
                        phase = 'manual_ready'
                        if handset:
        ...
        if proc is not None and proc.poll() is None:
            try:
                ...
                event('CLEANUP_FEEDBACK_ERROR ' + str(cleanup_error))
            finally:
                phase = 'exit'
        ...
        result = dict(dry=dry, manual=manual, handset=handset, direction=direction, strength=strength, seconds=seconds,
            ...
            command_scale=[1., 1., 1.] if him else [1.5, .5, .6],
            policy='HIM checkpoint 1500' if him else 'legacy', rl_zero_seconds=(.04 if dry else 1) if him else 5,
...
def check():
    assert sdk_feedback_error({'sdk_mode': {'updated': 10, 'state': 100}}, 10.1) is None
    assert sdk_feedback_error({'sdk_mode': {'updated': 10, 'state': 17}}, 10.1)
    assert sdk_feedback_error({'sdk_mode': {'updated': 10, 'state': 100}}, 10.6)
    assert sdk_feedback_error({}, 10)
    remote = HandsetInput()
...
if __name__ == '__main__':
    if sys.argv[1:] == ['--check']:
        check()
    elif len(sys.argv) == 3 and sys.argv[2] in ('--him-dry', '--him-stand'):
        main(Path(sys.argv[1]), sys.argv[2] == '--him-dry', him=True)
        ...
        main(Path(sys.argv[1]), sys.argv[2] == '--handset-dry', manual=True, handset=True, him=True)
