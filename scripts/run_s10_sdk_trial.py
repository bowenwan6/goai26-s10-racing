...
from s10_sdk_trial import manual_request, handset_request
...
def run(args, cancel=None, alive=None, report=print, command_source=None):
    ...
    if manual and command_source is None:
        raise ValueError('Manual mode requires a control source; open start_s10_speedturn.cmd')
    ...
    local_log = Path(__file__).resolve().parents[1] / 'artifacts/s10-sdk' / (args.host_alias or args.host) / run_id
    try:
        subprocess.run(['scp', *options, str(Path(__file__).with_name('s10_sdk_trial.py')),
                        target + ':' + str(root / 'supervisor.py')], check=True, **process_options)
        if cancel is not None and cancel.is_set():
            return
        mode = {'dry': '--dry', 'stand': '--run', 'left': '--left', 'right': '--right',
        ...
        for name in ('supervisor.log', 'result-' + suffix + '.json', 'deploy-' + suffix + '.log'):
            ...
            subprocess.run(['scp', *options, target + ':' + str(remote_log / name), str(local_log / name)],
                           check=not name.startswith('deploy-'), **process_options)
