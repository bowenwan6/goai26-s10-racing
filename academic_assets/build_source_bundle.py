...
BASE = Path('<home>/Documents/Projects/goai26')
...
evidence_files = {
    'implementation_detail_reference.md': ROOT / 'report_assets/archive/before_six_page_review/PROJECT_TECHNICAL_ZH.md',
...
metrics = {
    ...
    'successes_with_front_support_loss': sum(bool(r['success']) and not bool(r['drop_free']) for r in rows),
...
assert metrics['successes_with_front_support_loss'] == 18
...
for name in ('PROJECT_TECHNICAL_ZH.md', 'PROJECT_TECHNICAL_ZH.pdf', 'FACT_CHECK_ZH.md', 'REPORT_PENDING_ITEMS_ZH.md', 'ACADEMIC_REVIEW_ZH.md'):
    path = ROOT / name
    if path.exists():
...
add_file('poster_assets/img/course_height_overview.png', ROOT / 'poster_assets/img/course_height_overview.png')
add_file('poster_assets/generate_course_height_map.py', ROOT / 'poster_assets/generate_course_height_map.py')
...
manifest = {
    ...
    'cpp_excerpt_count': len(cpp_blocks),
    'yaml_excerpt_count': len(yaml_blocks),
    'cpp_excerpt_verified_ignoring_whitespace': True if cpp_blocks else None,
    'yaml_excerpt_verified': True if yaml_blocks else None,
    'files': entries,
}
