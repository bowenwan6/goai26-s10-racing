...
public:
    S10PolicyRunner(const std::string &policy_name, const std::string &policy_path) :
        ...
        dof_default_eigen_policy << 0.05, -0.35,  0.65, 
                                    -0.05, -0.35,  0.65,  
                                    0.05,  0.35, -0.65,  
                                    -0.05,  0.35, -0.65, 
                                    0.0, 0.0, 0.0, 0.0;
        dof_default_eigen_robot << 0.05, -0.35,  0.65, 0.0,
                                   -0.05, -0.35,  0.65, 0.0,
                                   0.05,  0.35, -0.65, 0.0,
                                   -0.05,  0.35, -0.65, 0.0;
        SetDecimation(4);
        ...
        kd_ = Vec4f(2, 2, 2, 0.8).replicate(4, 1);
    ...
    RobotAction getRobotAction(const RobotBasicState &ro, const UserCommand &uc) {
        ...
        Vec3f command = Vec3f(uc.forward_vel_scale * 1.5, uc.side_vel_scale * 0.5, uc.turnning_vel_scale * 0.6);
        ...
        if (run_cnt_ < 150){
            float ratio = run_cnt_/ 150.0;
            current_action_eigen *= ratio; 
            robot_action.kp = kp_ * (0.2 + 0.8 * ratio);
            robot_action.kd = kd_ * 1.5;
        }
        else{
            robot_action.kp = kp_;
            robot_action.kd = kd_;
