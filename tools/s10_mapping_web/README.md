...
实时高度图：[高度图入口](http://10.21.41.1:8080/heightmap)，沿用网页登录。支持立体 / 俯视、旋转和缩放；读取 106 官方 `/elevation_map_raw`，保留 200×200、5 cm 网格和未知区域，不启动 mapping。图源约 10 Hz，网页 1 Hz；定位未确认正常时提示异常并隐藏位置箭头，数据过期时隐藏旧图。

2026-09-13 实机确认 `traversability_estimation`、`accumulate_cloud` 已为 active，未重复启动；`mapping` 为 inactive。高程图依赖 `/LOC_BODY_POINTS` 和 `/ODOM`，不依赖 AGX 的 RealSense 与点云合并服务。此次约 26,518 / 40,000 格有效，厂商定位报告“定位丢失”，因此仅作为原始高程输出查看。连续热点 HTTP 采样验证帧时间前进；浏览器自动操作超时，未完成画面交互验收。

高度图读取由 `heightmap.py` 在 106 按需执行，通过现有受限 SSH 后端传到 AGX；只有访问 `/phone/heightmap` 才启动独立订阅，停止查看约 30 秒后退出。部署时将 `heightmap.py` 与 `robot_backend.py` 放到 106 的 `~/s10_mapping_web/`，将 `server.py`、`index.html`、`heightmap.html` 放到 AGX 同名目录，再重启现有用户网页服务。部署前文件与实测记录在 `artifacts/s10-48-heightmap-preview-20260913/`，不包含登录凭据。

离线检查：`python tools/s10_mapping_web/test_heightmap.py` 和 `python tools/s10_mapping_web/test_server.py`。解码检查覆盖列优先非方形矩阵、循环缓冲起点、偏移、无效值及截断。

停止 AGX 的额外合并 / 相机服务（需要有 sudo 权限的账号）：

```bash
sudo systemctl stop rslidar-merge.service s10-realsense.service
# 恢复
sudo systemctl start rslidar-merge.service s10-realsense.service
```

`stop` 只停止本次运行，不撤销已配置的开机自启。合并服务只做计算，停止后减少计算负载；RealSense 停止采集可待机，仍由 USB 供电。Airy 驱动停止只停主机接收 / 解析，不等于雷达硬件待机；厂商 Standby 模式才停止电机和激光。参考 [RealSense 待机说明](https://support.realsenseai.com/hc/en-us/community/posts/360033687973-Power-Saving-with-D435)、[Airy 手册](https://robosense-robotics.github.io/product-manual/en/Airy/)。本次未停止这些传感器，也未改变开机设置。
